<script>
	jQuery(document).ready(function($) {
		$("#w2dc-field-input-<?php echo $content_field->id; ?>").datepicker({
			<?php if (function_exists('is_rtl') && is_rtl()): ?>isRTL: true,<?php endif; ?>
			showButtonPanel: true,
			dateFormat: '<?php echo $dateformat; ?>',
			onSelect: function(dateText) {
				var sDate = $("#w2dc-field-input-<?php echo $content_field->id; ?>").datepicker("getDate");
				if (sDate) {
				...
				$("input[name=w2dc-field-input-<?php echo $content_field->id; ?>]").val(tmstmp_str);
			}
		});
		<?php
		if ($lang_code = w2dc_getDatePickerLangCode(get_locale())): ?>
		$("#w2dc-field-input-<?php echo $content_field->id; ?>").datepicker($.datepicker.regional[ "<?php echo $lang_code; ?>" ]);
		<?php endif; ?>
		
		<?php if ($content_field->value['date']): ?>
		$("#w2dc-field-input-<?php echo $content_field->id; ?>").datepicker('setDate', $.datepicker.parseDate('dd/mm/yy', '<?php echo date('d/m/Y', $content_field->value['date']); ?>'));
		<?php endif; ?>
		$("#reset_date").click(function() {
			$.datepicker._clearDate('#w2dc-field-input-<?php echo $content_field->id; ?>');
		})
	});
</script>
<div class="w2dc-form-group w2dc-field w2dc-field-input-block w2dc-field-input-block-<?php echo $content_field->id; ?>">
	...
	<div class="w2dc-col-md-10">
		<div class="w2dc-row w2dc-form-group">
			<div class="w2dc-col-md-4 w2dc-has-feedback">
				<input type="text" id="w2dc-field-input-<?php echo $content_field->id; ?>" class="w2dc-field-input-datetime w2dc-form-control" />
				<span class="w2dc-glyphicon w2dc-glyphicon-calendar w2dc-form-control-feedback"></span>
				<input type="hidden" name="w2dc-field-input-<?php echo $content_field->id; ?>" value="<?php echo esc_attr($content_field->value['date']); ?>"/>
			</div>
			<div class="w2dc-col-md-3">
				<input type="button" class="w2dc-btn w2dc-btn-primary" id="reset_date" value="<?php esc_attr_e('reset date', 'W2DC')?>" />
			</div>
		</div>
		<?php if ($content_field->is_time): ?>
		<div class="w2dc-row">
			<label class="w2dc-col-md-1"><?php _e('Time:', 'W2DC'); ?></label>
			<div class="w2dc-col-md-3">
