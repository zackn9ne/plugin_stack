		...
		if ($paypal->validateIpn()) {
			if (strcasecmp($paypal->ipnData['business'], get_option('w2dc_paypal_email')) === 0) {
				if (is_unique_transaction($paypal->ipnData['txn_id'])) {
					...
					if (($invoice = getInvoiceByID($invoice_id)) && $invoice->taxesPrice(false) && floatval($paypal->ipnData['mc_gross']) == $invoice->price && $paypal->ipnData['mc_currency'] == get_option('w2dc_payments_currency')) {
