...
class w2dc_avg_rating {
	...
	public function __construct($post_id) {
		...
		if (version_compare($wp_version, '4.0', '<'))
			$like = like_escape(W2DC_RATING_PREFIX);
		else
...
class w2dc_rating {
	...
	public function __construct($row) {
		...
		if (!self::$validation)
			self::$validation = new form_validation();
...
function w2dc_flush_ratings($post_id) {
	global $wpdb, $wp_version;
	...
	return ($wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->postmeta} WHERE post_id=%d AND (meta_key=%s OR meta_key LIKE %s)", $post_id, W2DC_AVG_RATING_KEY, $like.'%')) !== false) ? true : false;
}
