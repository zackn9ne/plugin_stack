		...
		function render( $args, $content = '' ) {
			...
			if ( self::$bg_type == 'video' && ! empty( self::$args['video_url'] ) ) {
				...
				if ( $video_url['type'] == 'youtube' ) {
					$outer_html .= "<div style='visibility: hidden' id='video-" . self::$parallaxID ++ . "' data-youtube-video-id='" . $video_url['id'] . "' data-mute='" . ( self::$args['video_mute'] == 'yes' ? 'true' : 'false' ) . "' data-loop='" . ( self::$args['video_loop'] == 'yes' ? 1 : 0 ) . "' data-loop-adjustment='" . self::$args['video_loop_refinement'] . "' data-video-aspect-ratio='" . self::$args['video_aspect_ratio'] . "' data-overlay-opacity='" . self::$args['overlay_opacity'] . "'><div id='video-" . self::$parallaxID ++ . "-inner'></div></div>";
